...
pred_file = 'D:/PySIM/scunet_2D/U2Os_Actin_5-7-'+str(Nthe)+'x'+str(Nphi)+'.mat'
gt = loadmat(gt_file)['reconOb'].astype(float)
...
# col_VOL = ['MSE-1', 'SSIM-1', 'PSNR-1']
# vol_list = []
# for i in range (3):
#     vol_list = str(volmetrics[i])


# vol_assess = pd.DataFrame(list(volmetrics), columns=col_VOL) 
# vol_assess.to_csv('metrics_Vol_2d'+'_'+str(Nthe)+'_'+str(Nphi)+'.csv')
