...
def get_images():
    ...
    for i in tqdm(range(1, limit + 1, 1)):
        ...
        inp_set = np.array(inp_set).astype('double')
        inp_set = np.expand_dims(inp_set, axis=0)
        inp_set = torch.from_numpy(inp_set).double()
...
def save_pred(model, data):
    ...
    for image, file_path in data:
        ...
        pred_heads = ['Pred-Min', 'Pred-Max']
        pred_norm = pd.DataFrame(list(zip(pred_min, pred_max)), columns=pred_heads)
