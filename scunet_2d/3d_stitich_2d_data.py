...
def get_summed_img(img_crops_arr, step_h, step_w):
    ...
    img_summed = np.zeros( (img_summed_h , img_summed_w) , dtype='double')
...
def get_averaging_mask(img_crops_arr, step_h, step_w):
    ...
    mask = np.zeros( (mask_h , mask_w) , dtype='double')
    for idx_h in range(num_crops_h):
        for idx_w in range(num_crops_w):
            offset_h = idx_h*step_h
            offset_w = idx_w*step_w
...
def matsave_stitched_img(stitched_img, key_name='crop_g', out_filepath='2D_U2Os_Actin_'+str(Nthe)+'x'+str(Nphi)+'.mat'):
