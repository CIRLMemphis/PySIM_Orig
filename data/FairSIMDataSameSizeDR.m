...
for ind = 1:length(datasets)
    ...
    gCutDR = zeros(length(regionX{ind}),length(regionY{ind}),length(slides{ind}), size(DRThetaPhi, 1));
    for zInd = 1:Z
        for DRInd = 1:size(DRThetaPhi,1)
            temp = fadeBorderCos(gUp(:,:,zInd,DRThetaPhi(DRInd, 1), DRThetaPhi(DRInd, 2)), 15*2);
            gCutDR(:,:,zInd,DRInd) = temp(regionX{ind}, regionY{ind});
        end
    end

    endRawCnt = NNCrop(gCutDR, 128*2, 128*2, 1, 32*2, 32*2, 1, 'in/inFairSIM', fileCnt);
